...
# Hyperparameters for ARS (specifically tuned for Hardcore)
HP = {
    ...
    "episodes": 2000,       # Increased episodes for hardcore terrain
    "episode_length": 2000, # Increased max steps for complex terrain
}
...
def explore(env, normalizer, policy, direction=None, delta=None):
    ...
    while not done and steps < HP["episode_length"]:
        ...
        # Clip actions to [-1, 1] as required by BipedalWalker
        action = np.clip(action, -1.0, 1.0)
        state, reward, terminated, truncated, info = env.step(action)
        reward = max(-1, min(1, reward))  # Clipping reward for stability during training
        total_reward += reward
        steps += 1
...
def train():
    ...
    weights_path = "Bipedal_Walker/bipedal_walker_hardcore_ars.pkl"
    if os.path.exists(weights_path):
        with open(weights_path, "rb") as f:
            policy.theta, normalizer.mean, normalizer.var = pickle.load(f)
        print("Loaded existing weights.")

    print(f"Training ARS for Bipedal Walker Hardcore ({HP['episodes']} episodes)...")
...
def test():
    ...
    print("Testing the trained agent on Hardcore terrain...")
    state, info = env.reset()
    done = False
    total_reward = 0
    while not done:
...
if __name__ == "__main__":
    if "--train" in sys.argv or not os.path.exists("Bipedal_Walker/bipedal_walker_hardcore_ars.pkl"):
        train()
        test()
    else:
        test()
